...
/* UDP port for LAN discovery. Distinct from the game's TCP port (22777) and
 * the test harness base port (22778). */
...
/* Open the UDP responder socket bound to LAN_DISCOVERY_PORT. Add it to a select
 * set; when readable, call lan_discovery_answer(). Returns TCPME_INVALID_SOCKET
 * on failure (LAN discovery may be treated as optional). */
tcpme_socket_t lan_discovery_open_responder(void);
...
/* Broadcast a discovery query. Returns true on success. */
